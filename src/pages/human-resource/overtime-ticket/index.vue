<template>
  <van-tabs v-model:active="active">
    <van-tab title="处理中" name="handling">
      ...
      <van-list v-model:loading="handling.loading" :finished="handling.finished" finished-text="没有更多了"
        ...
        <van-cell v-for="item in handling.list" :key="item.id" :title="approvedTitle(item)" is-link
          :url="`overtime-ticket/detail?id=${item.id}`" />
      </van-list>
    </van-tab>
    ...
    <van-floating-bubble axis="lock" icon="plus" @click="addOvertimeTicket" />
  </van-tabs>
</template>
...
function searchHandling(reset: boolean) {
  ...
  Api.req('/overtime-ticket/query').query(param).success((data: any[]) => {
    data = data ?? []
...
const addOvertimeTicket = () => {
  router.push('/human-resource/overtime-ticket/form');
}
