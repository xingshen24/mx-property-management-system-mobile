<template>
  <van-search v-model="keywords" placeholder="请输入员工名" input-align="center" @search="setKeywordsAndSearch" />
...
const empName = (item: any) => {
  return `【${item.deptName} - ${item.jobName}】${item.empName}`;
}
...
function search(reset: boolean) {
  loading.value = true
  const param = { ...form }
  Api.req('/employee/query').query(param).success((data: any[]) => {
    data = data ?? []
