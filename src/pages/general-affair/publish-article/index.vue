<template>
  <van-search v-model="keywords" placeholder="文章标题/编号/主题词/上报/内容" input-align="center" @search="setKeywordsAndSearch" />
  <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
    <van-cell v-for="item in list" :key="item.id" :title="receiveItemTitle(item)" is-link
      :url="`publish-article/detail?id=${item.id}`" />
  </van-list>
</template>
...
function search(reset: boolean) {
  loading.value = true
  const param = { ...form }
  Api.req('/publish-article/query').query(param).success((data: any[]) => {
    data = data ?? []
