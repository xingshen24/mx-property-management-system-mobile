<template>
  <van-search v-model="keywords" placeholder="请输入部门/议案标题/内容" input-align="center" @search="setKeywordsAndSearch" />
  <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
    <van-cell v-for="item in list" :key="item.id" :title="proposalTitle(item)" is-link
      :url="`proposal/detail?id=${item.id}`" />
  </van-list>
</template>
...
const proposalTitle = (item: any) => {
  return `【${item.deptName}】${item.title}`;
}
...
function search(reset: boolean) {
  loading.value = true
  const param = { ...form }
  Api.req('/proposal/query').query(param).success((data: any[]) => {
    data = data ?? []
